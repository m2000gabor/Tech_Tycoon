...
public abstract class AbstractAssistant {
    ...
    abstract Wrapped_DeviceAndCompanyList work(List<Company> companyList, List<Device> deviceList, List<Device> myDevices, Company myCompany, Wrapped_DeviceAndCompanyList ret);
    ...
    static int max(Device[] list, int type) {
        int maxInd = 0;
        for (int i = 0; i < list.length; i++) {
            if (list[i].getFieldByNum(type) > list[maxInd].getFieldByNum(type)) {
                maxInd = i;
            }
        }
